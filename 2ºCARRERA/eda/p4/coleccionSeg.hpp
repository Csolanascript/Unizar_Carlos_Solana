...
//colección igual a c.
template<typename segm,typename val> void aniadir(coleccionSeg<segm,val>&c, const segm& s, const val& v, bool& error);
...
template<typename segm, typename val> bool borrar_REC(typename coleccionSeg<segm,val>::abb*& a, const segm& s);

template<typename segm, typename val> bool borrarMax(typename coleccionSeg<segm,val>::abb*& a, segm& s, val& v);
...
template<typename segm,typename val> struct coleccionSeg {
    ...
    private:
        ...
        int tamanio;
        Pila<abb*> puntero;
        friend bool seSolapa_REC<segm,val>(typename coleccionSeg<segm,val>::abb*& a, const segm& s);

        friend bool aniadir_REC<segm,val>(typename coleccionSeg<segm,val>::abb*& a, const segm& s, const val& v);

        friend bool actualizar_REC<segm,val>(typename coleccionSeg<segm,val>::abb*& a, const segm& s, const val& v);

        friend bool esta_REC<segm,val>(typename coleccionSeg<segm,val>::abb*& a, const segm& s);

        friend bool obtener_REC<segm,val>(typename coleccionSeg<segm,val>::abb*& a, const segm& s, val& v);

        friend bool borrar_REC<segm,val>(typename coleccionSeg<segm,val>::abb*& a, const segm& s);

        friend bool borrarMax<segm,val>(typename coleccionSeg<segm,val>::abb*& a, segm& s, val& v);
    ...
    friend void aniadir<segm,val>(coleccionSeg<segm,val>&c, const segm& s, const val& v,bool& error);
...
template<typename segm,typename val> bool aniadir_REC(typename coleccionSeg<segm,val>::abb*&a, const segm& s, const val& v) {
    if(a == nullptr) {
        ...
        if(anterior(s,a->segmento)){
            if(!seSolapa_REC<segm,val>(a->derecha,s)) {// comprueba solapes por la derecha
                return aniadir_REC(a->izquierda,s,v);
            }
        }
        else {
            if(!seSolapa_REC<segm,val>(a->izquierda,s)) {// comprueba solapes por la izquierda
                return aniadir_REC(a->derecha,s,v);
            }
        }
    }
    return false;
...
template<typename segm,typename val> bool actualizar_REC(typename coleccionSeg<segm,val>::abb*& a, const segm& s, const val& v){
    ...
    if(anterior(s,a->segmento)){
        return actualizar_REC<segm,val>(a->izquierda,s,v);
    }
    else {
        return actualizar_REC<segm,val>(a->derecha,s,v);
    }
}
//Si en c hay algún elemento con segm igual a s, sea (s, v’), devuelve una colección igual a la
//resultante de sustituir dicho elemento por (s, v) en la colección c, no definida si en c no hay ningún elemento con segm igual a s
...
template<typename segm,typename val> bool actualizar(coleccionSeg<segm,val>& c, const segm& s, const val& v) {
    if(!esVacia(c)){
        return actualizar_REC<segm,val>(c.raiz,s,v);
    }
    else{
        return false;
    }
...
template<typename segm,typename val> bool esta(coleccionSeg<segm,val>&c, const segm& s) {
    if(esVacia(c)){
        ...
        return esta_REC<segm,val>(c.raiz,s);
    }
}
...
template<typename segm,typename val> bool esta_REC(typename coleccionSeg<segm,val>::abb*& a, const segm& s){
    ...
    if(anterior(s,a->segmento)){
        return esta_REC<segm,val>(a->izquierda,s);
    }
    else {
        return esta_REC<segm,val>(a->derecha,s);
    }
}
...
template<typename segm, typename val>
bool seSolapa_REC(typename coleccionSeg<segm,val>::abb*& a, const segm& s){
    if(a == nullptr){
        //cout <<"ENTRO" << endl;
        return false;
    }
    else{
        /*cout << anio(obtenerInicio(s)) << "-" << mes(obtenerInicio(s)) << "-" << dia(obtenerInicio(s)) << " " << hora(obtenerInicio(s)) << ":" << minuto(obtenerInicio(s)) << " <---> ";
        cout << anio(obtenerFinal(s)) << "-" << mes(obtenerFinal(s)) << "-" << dia(obtenerFinal(s)) << " " << hora(obtenerFinal(s)) << ":" << minuto(obtenerFinal(s)) << " ES EL SEGMENTO QUE BUSCARA SOLAPE CON EL SEGMENTO DEL ARBOL: " << endl;
        cout << anio(obtenerInicio(a->segmento)) << "-" << mes(obtenerInicio(a->segmento)) << "-" << dia(obtenerInicio(a->segmento)) << " " << hora(obtenerInicio(a->segmento)) << ":" << minuto(obtenerInicio(a->segmento)) << " <---> ";
        cout << anio(obtenerFinal(a->segmento)) << "-" << mes(obtenerFinal(a->segmento)) << "-" << dia(obtenerFinal(a->segmento)) << " " << hora(obtenerFinal(a->segmento)) << ":" << minuto(obtenerFinal(a->segmento)) << endl;
        */
        if(solapados(s,a->segmento)){
            //cout << "ENTROoooo";
            return true;
        }
        else{
            //cout <<"ENTRO2" << endl;
            return (seSolapa_REC<segm,val>(a->izquierda, s) || seSolapa_REC<segm,val>(a->derecha, s));
        }
    }
}

template<typename segm, typename val>
bool borrarMax(typename coleccionSeg<segm,val>::abb*& a, segm& s, val& v){
    if(a->derecha == nullptr){
        s = a->segmento;
        v = a->valor;
        typename coleccionSeg<segm,val>::abb* aux;
        aux=a;
        a=a->izquierda;
        delete(aux);
        return true;
        ...
        return borrarMax<segm,val>(a->derecha, s,v);
    }
}

template<typename segm, typename val>
bool borrar_REC(typename coleccionSeg<segm,val>::abb*& a, const segm& s){
    if (a != nullptr){
        if(iguales((a->segmento), s)){
            if(a->izquierda == nullptr){
                typename coleccionSeg<segm,val>::abb* aux = a;
                a=a->derecha;
                delete(aux);
                return true;
            }
            else{
                return borrarMax<segm,val>(a->izquierda, a->segmento,a->valor);
            }
        }

        if(!anterior(s,a->segmento)){
            return borrar_REC<segm,val>(a->derecha,s);
        }
        if(anterior(s,a->segmento)){
	        return borrar_REC<segm,val>(a->izquierda,s);
        }
    }
    return false;
}
...
bool borrar(coleccionSeg<segm,val>& c, const segm& s){
    bool borrado = borrar_REC<segm,val>(c.raiz,s);
    if(borrado) {
        c.tamanio--;
    }
    return borrado;
