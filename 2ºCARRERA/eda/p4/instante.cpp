...
bool anterior(const instante& i1, const instante& i2){
    cout << "INSTANTE 1 : " << i1.anio << "-" << i1.mes << "-" << i1.dia << " " << i1.hora << ":" << i1.minuto << endl;
    cout << "INSTANTE 2 : " << i2.anio << "-" << i2.mes << "-" << i2.dia << " " << i2.hora << ":" << i2.minuto << endl;
    bool es_anterior = false;
    if (i1.anio < i2.anio)
    {
        cout << "ANTERIOR CASO 2." << endl;
        return true;
    }
    if (i1.anio > i2.anio)
    {
        cout << "POSTERIOR CASO 2." << endl;
        return false;
    }
    if (i1.mes < i2.mes)
    {
        cout << "ANTERIOR CASO 3." << endl;
        return true;
    }
    if (i1.mes > i2.mes)
    {
        cout << "POSTERIOR CASO 3." << endl;
        return false;
    }
    if (i1.dia < i2.dia)
    {
        cout << "ANTERIOR CASO 4." << endl;
        return true;
    }
    if (i1.dia > i2.dia)
    {
        cout << "POSTERIOR CASO 4." << endl;
        return false;
    }
    if (i1.hora < i2.hora)
    {
        cout << "ANTERIOR CASO 5." << endl;
        return true;
    }
    if (i1.hora > i2.hora)
    {
        cout << "POSTERIOR CASO 5." << endl;
        return false;
    } 
    if (i1.minuto < i2.minuto)
    {
        cout << "ANTERIOR CASO 6." << endl;
        return true;
    }
    if (i1.minuto > i2.minuto)
    {
        cout << "POSTERIOR CASO 6." << endl;
        return false;
    }
    else {
        cout << "NO ES ANTERIOR. " << endl;
    }
    return false;
